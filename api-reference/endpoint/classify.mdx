...
Everything sits under `data.classification`, the same key `/enrich` uses — so `data.classification.category` reads identically from either endpoint.
...
const { category, subcategory } = data.classification;
...
classification = r.json()["data"]["classification"]
...
The `classification.iab_id` field follows the IAB Content Taxonomy 3.0 format:
