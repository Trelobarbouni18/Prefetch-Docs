...
You need an API key. Get one from the [dashboard](https://prefetch.io/dashboard).
...
print(body["data"]["classification"]["category"])  # "Personal Finance"
```
...
const json = await res.json() as {
  ...
  data?: {
    url: string;
    domain: string;
    classification: { category: string; subcategory: string | null; description: string };
  };
  error?: string;
};

if (!json.success) throw new Error(json.error);
console.log(json.data!.classification.category);
```
...
<CardGroup cols={2}>
  ...
  <Card title="MCP server" icon="plug" href="/mcp/overview">
    Let Claude or Cursor call these endpoints for you.
  </Card>
</CardGroup>
